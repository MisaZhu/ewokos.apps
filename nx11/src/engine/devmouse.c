...
#if EWOKOS
	/*
	 * The EwokOS xserver draws the desktop cursor, so Nano-X must not draw
	 * one of its own into the virtual screen. Leaving curvisible at 0 also
	 * turns GdCheckCursor, GdEraseCursor and GdFixCursor into no-ops.
	 */
	return curvisible;
#endif
#if !USE_VGA_XOR_CURSOR
...
{
#if EWOKOS
	/* paired with the GdShowCursor no-op above, keeps curvisible pinned at 0*/
	return curvisible;
#endif
#if !USE_VGA_XOR_CURSOR
	MWPIXELVALHW *	saveptr;
