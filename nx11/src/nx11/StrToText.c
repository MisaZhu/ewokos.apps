...
#if EWOKOS
/*
 * EwokOS libc has no wchar/locale support, and its stdio.h is incompatible
 * with newlib's <wchar.h> (that header expects newlib's __FILE).  Provide the
 * three conversions this file needs instead, mapping wide characters to and
 * from Latin-1: X clients only ever hand us 8-bit strings here, so the
...
wctob(wchar_t c)
{
	/* wchar_t is unsigned on ARM and signed elsewhere, so compare through an
	   unsigned type: a negative value and one above 0xff both become EOF*/
	unsigned int v = (unsigned int)c;

	return (v <= 0xffu)? (int)v: EOF;
}

static wchar_t
btowc(int c)
{
	return (c == EOF)? (wchar_t)0: (wchar_t)(unsigned char)c;
}
#else
#include <wchar.h>
#endif
