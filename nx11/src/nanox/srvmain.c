...
#include "osdep.h"
#if UNIX && HAVE_SELECT
#include <sys/select.h>		/* fd_set for GrPrepareSelect/GrServiceSelect*/
#endif
#if EWOKOS
#include "ewokos.h"		/* EwokOS xwin bridge hooks*/
#endif
...
#if EWOKOS
	/*
	 * EwokOS input is not file-descriptor based, so select() cannot be used.
	 * The bridge blocks on (or polls) the xserver's per-process event node,
	 * drains it into the mouse/keyboard queues and services the timers.
	 */
	nx11x_select(timeout);
#else
	fd_set	rfds;
	int 	e;
	int	setsize = 0;
...
#if EWOKOS
	/*
	 * EwokOS _exit() is a no-op stub (system/basic/libc/libgloss/syscalls.c),
	 * so the linked server would keep running - and its event loop keep
	 * spinning - after "terminating".  exit() issues the SYS_EXIT syscall that
	 * really tears the process down.  GsTerminate() is not registered with
	 * atexit() on this platform (only MSDOS does), so exit() cannot recurse.
	 */
	exit(0);
#else
	_exit(0);
