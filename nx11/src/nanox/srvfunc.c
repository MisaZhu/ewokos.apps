...
#if EWOKOS
	/* bring the matching EwokOS xwin to the front too*/
	nx11x_win_raised(wp);
#endif
...
#if EWOKOS
	/* every GrMoveWindow/GrReparentWindow path funnels through here, so this
	   single hook keeps the matching EwokOS xwin in step (children are not in
	   the bridge's list and are ignored)*/
	nx11x_win_moved(wp);
#endif
}
...
#if EWOKOS
	/* resize the matching EwokOS xwin before the expose pass so the mirror
	   lands in a workspace that is already the right size*/
	nx11x_win_resized(wp);
#endif
...
#if EWOKOS
	/* give the matching EwokOS xwin the desktop focus as well*/
	nx11x_win_focused(wp);
#endif
...
#if EWOKOS
	/* push the new title/style/bordersize out to the EwokOS xwin*/
	nx11x_win_props(wp);
#endif
