...
#if EWOKOS
	/* hide the matching EwokOS xwin (skip the temporary unmap GrMoveWindow
	   performs, the window is re-realized right after)*/
	nx11x_win_unrealize(wp, temp_unmap);
#endif
...
#if EWOKOS
	/* create/show the matching EwokOS xwin before wp is reused for the
	   children below*/
	nx11x_win_realize(wp);
#endif
...
#if EWOKOS
	/* drop the matching EwokOS xwin first, wp is freed at the end*/
	nx11x_win_destroy(wp);
#endif
