...
#if EWOKOS
/*
 * EwokOS / NX11 event wait.
 *
 * On EwokOS the Nano-X server is linked into the process (NONETWORK); its
 * event source is the xserver's per-process event node, not a socket.  The X
 * connection number handed out by GrOpen() is therefore a dummy fd and the
 * stock select()/poll() loop below would block forever on a readability that
 * never arrives.
 *
 * nx11x_select() is the very primitive the Nano-X server uses inside
 * GsSelect(): it flushes pending drawing, turns queued mouse/keyboard/expose
 * input into client events, then blocks on (or polls) the xserver event node
 * for exactly the requested time.  Its timeout uses nano-X units:
 *   0  -> block until an event arrives  (GR_TIMEOUT_BLOCK)
 *  -1  -> poll and return immediately    (GR_TIMEOUT_POLL)
 *  >0  -> wait at most that many milliseconds
 * Afterwards we drain the X queue through FLTK's normal do_queued_events().
 *
 * Correct for single-threaded FLTK programs only (Fl::lock() is inert because
 * HAVE_PTHREAD is 0 in config.h), which is the whole use case on EwokOS.
 */
extern "C" void nx11x_select(int timeout);
#define NX11_TIMEOUT_BLOCK 0
#define NX11_TIMEOUT_POLL  (-1)

int fl_wait(double time_to_wait) {
  if (fl_display && XQLength(fl_display)) {do_queued_events(); return 1;}

  int ms;
  if (time_to_wait >= 2147483.648) ms = NX11_TIMEOUT_BLOCK;
  else { ms = int(time_to_wait*1000 + .5); if (ms <= 0) ms = NX11_TIMEOUT_POLL; }
...
// XQLength() -> XEventsQueued(QueuedAfterReading) already polls Nano-X once
// (GrPeekEvent pumps with GR_TIMEOUT_POLL when the queue is empty), so no
// extra blocking call is required here.
int fl_ready() {
  return XQLength(fl_display) ? 1 : 0;
}
