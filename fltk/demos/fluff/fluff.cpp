...
#include <stdio.h>
#include <stdlib.h>		/* EwokOS: no libstdc++ <cstdlib> wrapper */
...
/* EwokOS: no LFS 64-bit stat variants; map onto plain struct stat / lstat. */
#define stat64 stat
...
/* EwokOS port: Home/trash defaults are filled from $HOME of the logged-in
   user in main() (xsession/login set HOME from the passwd entry, e.g.
   /home/misa); upstream's hardcoded Tiny Core path /home/tc does not exist. */
char TrashbinPath_str[MAXFPATHLEN] = "/.trash";
char Loc1Label_str[MAXALBLLEN]     = "Home";
char Loc1Path_str[MAXFPATHLEN]     = "/";
...
int File_Detail_List_Browser::item_number_under_mouse(int mouse_y) {
	...
	//~ printf("Mouse event y %d is at widget y %d considering scroll pos %d is in list line %d, which goes to y %d\n",
			//~ mouse_y, target_y, position(), line, accum_y);
	return line;
}
       ...
       case FL_FOCUS:
        ...
        case FL_RELEASE:
			//~ printf("RELEASE event in file list, ey %d, bY %d, vpos %d, wgt_y %d, row %d\n", 
					//~ ey,  y(), File_Detail_List_Browser::position(),wgt_y, event_row); fflush(0); 
...
void begin_renaming(void)
{
    ...
    if (MainWnd_p->last_click_browser == LASTCLICK_IN_LIST) {
        ...
        rx = 4 + p->x();
        ry = p->y() + p->item_y_coordinate(p->value()) - p->position();
    }
    else {
...
void init_dir_watching(void) {
    ...
    if (-1 == DirWatch_fd) {
        /* EwokOS port: no kernel inotify; degrade silently and let the main
           loop poll for changes instead of alarming the user with a dialog. */
        printf("fluff: inotify unavailable, falling back to polling\n");
    }
}
...
int main(int argc, char** argv) 
{
    ...
    /* EwokOS port: point the Home location and trashbin at the current
       user's $HOME instead of the hardcoded /home/tc. */
    {
        const char *hp = getenv("HOME");
        if (hp && *hp) {
            snprintf(Loc1Path_str, sizeof(Loc1Path_str), "%s", hp);
            snprintf(TrashbinPath_str, sizeof(TrashbinPath_str), "%s/.trash", hp);
        }
    }

    load_configuration();
    if (mkdir(TrashbinPath_str, DEFAULT_DIR_PERMS) != 0 && errno != EEXIST) {
        struct stat tb_st;
        if (stat(TrashbinPath_str, &tb_st) != 0 || !S_ISDIR(tb_st.st_mode)) {
            /* EwokOS port: $HOME may not be writable; fall back to a temp
               trashbin silently instead of alarming the user with a dialog. */
            printf("fluff: cannot create trashbin '%s', using /tmp/.trash\n",
                   TrashbinPath_str);
            snprintf(TrashbinPath_str, sizeof(TrashbinPath_str), "/tmp/.trash");
            mkdir(TrashbinPath_str, DEFAULT_DIR_PERMS);
        }
    }
...
//    MainWnd_p->show();
    ...
    while(Running && (0 <= Fl::wait(0.5))) {
        ...
        /* EwokOS port: no kernel inotify -> poll for external filesystem
           changes every ~3s by arming the existing resync path. */
        if (DirWatch_fd < 0 && !OperationInProgress
            && systemtime_as_real() > poll_at_rsec) {
            TimeToResync_rsec = systemtime_as_real();
            poll_at_rsec = systemtime_as_real() + 3.0;
        }
