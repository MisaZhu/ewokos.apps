...
static disk_factory *disk_factories[] = {
	/* sparsebundle/VHD support not available on EwokOS */
	NULL
};
...
size_t Sys_read(void *arg, void *buffer, loff_t offset, size_t length)
{
	...
	// Read data (EwokOS: file reads may return short, loop)
	char *p = (char *)buffer;
	size_t done = 0;
	while (done < length) {
		ssize_t r = read(fh->fd, p + done, length - done);
		if (r <= 0)
			break;
		done += (size_t)r;
	}
...
size_t Sys_write(void *arg, void *buffer, loff_t offset, size_t length)
{
	...
	// Write data (EwokOS: file writes may return short, loop)
	const char *p = (const char *)buffer;
	size_t done = 0;
	while (done < length) {
		ssize_t r = write(fh->fd, p + done, length - done);
		if (r <= 0)
			break;
		done += (size_t)r;
	}
