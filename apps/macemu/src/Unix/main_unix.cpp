...
int main(int argc, char **argv)
{
	...
	for (int i=1; i<argc; i++) {
		if (strcmp(argv[i], "--help") == 0) {
			...
			argv[i++] = NULL;
			if (i < argc) {
				ROMBreakpoint = (uint32)strtoul(argv[i], NULL, 16);
				argv[i] = NULL;
			}
#ifdef ENABLE_MON
	...
	// EwokOS: file reads may return short, loop until the ROM is complete
	ssize_t rom_got = 0;
	while (rom_got < (ssize_t)ROMSize) {
		ssize_t r = read(rom_fd, ROMBaseHost + rom_got, ROMSize - rom_got);
		if (r <= 0)
			break;
		rom_got += r;
	}
	if (rom_got != (ssize_t)ROMSize) {
...
#ifdef USE_EWOK_XWIN
	// EwokOS xwin backend (like minivmac): the 68k core runs in a
	// pthread while the x event loop runs on this (main) thread
	extern void VideoRunLoop(void);
	VideoRunLoop();
#else
	Start680x0();
#endif
...
struct B2_mutex {
	B2_mutex() { 
	    // NOTE: EwokOS pthreads do not support mutex attributes
