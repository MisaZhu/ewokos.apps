...
monitor_desc::monitor_desc(const vector<video_mode> &available_modes, video_depth default_depth, uint32 default_id) : modes(available_modes)
{
	...
	// Frame buffer base not published yet; the first cscSetMode must run
	// switch_mode() even if the requested mode equals the default (see
	// driver_control), otherwise the guest keeps drawing at address 0
	mac_frame_base = 0;
}
...
int16 monitor_desc::driver_open(void)
{
	...
	// Note: the frame buffer is allocated and published by switch_mode()
	// on the guest's first cscSetMode (see driver_control), not here.
...
int16 monitor_desc::driver_control(uint16 code, uint32 param, uint32 dce)
{
	switch (code) {
		...
		case cscSetMode: {		// Set color depth
			...
			// Also run the switch when the base was never published
			// (mac_frame_base == 0): switch_mode() is what publishes the
			// frame buffer address to the guest (DCE, param block,
			// ScrnBase/CrsrBase, MainDevice pixmap).  Skipping it because
			// the first requested mode equals the default leaves the guest
			// drawing at address 0, corrupting low memory.
			if (mode != current_apple_mode || mac_frame_base == 0) {
