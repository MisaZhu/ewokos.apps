...
bool CheckROM(void)
{
	...
	if (ROMVersion != ROM_VERSION_32) {
		printf("ERROR: ROM version 0x%04x detected, need a 32-bit clean\n"
		       "Mac II ROM (version 0x%04x, e.g. Mac IIci/IIfx/Quadra, 512KB/1MB)\n",
	...
	if (ROMVersion != ROM_VERSION_CLASSIC && ROMVersion != ROM_VERSION_32) {
		printf("ERROR: ROM version 0x%04x detected, need an SE/Classic ROM\n"
		       "(version 0x%04x, 256/512KB) or a 32-bit clean Mac II ROM\n"
		       "(version 0x%04x, e.g. Mac IIci/IIfx/Quadra, 512KB/1MB).\n"
		       "Mac 128K/512K/Plus ROMs (like the one used by mini vMac)\n"
		       "are not supported by Basilisk II.\n",
		       ROMVersion, ROM_VERSION_CLASSIC, ROM_VERSION_32);
		return false;
	}
	return true;
#endif
