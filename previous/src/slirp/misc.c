...
{
	return inet_pton(AF_INET, cp, &ia->s_un.s_addr);
}
#endif
...
void getouraddr()
{
    ...
    if (our_addr.s_un.s_addr == 0)
        our_addr.s_un.s_addr = loopback_addr.s_un.s_addr;
}
...
void snooze_hup(int num)
{
	...
	if (slirp_socket_passwd) {
		...
		sock_in.sin_addr.s_un.s_addr = slirp_socket_addr;
