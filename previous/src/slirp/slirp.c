    ...
    while (fgets(buff, 512, f) != NULL) {
        if (sscanf(buff, "nameserver%*[ \t]%256s", buff2) == 1) {
            if (!inet_aton(buff2, &tmp_addr))
                continue;
            if (tmp_addr.s_un.s_addr == loopback_addr.s_un.s_addr)
                tmp_addr = our_addr;
...
int slirp_init(void)
{
	...
	alias_addr.s_un.s_addr = special_addr.s_un.s_addr | htonl(CTL_ALIAS);
	getouraddr();
    return 0;
}
...
{
    if (is_udp) {
        if (!udp_listen(htons(host_port), guest_addr.s_un.s_addr, 
                        htons(guest_port), 0))
            return -1;
    } else {
        if (!solisten(htons(host_port), guest_addr.s_un.s_addr, 
                      htons(guest_port), 0))
            return -1;
    }
    return 0;
