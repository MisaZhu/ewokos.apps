	...
	u_int fport;
{
	...
	for (so = head->so_next; so != head; so = so->so_next) {
		if (so->so_lport == lport && 
		    so->so_laddr.s_un.s_addr == laddr.s_un.s_addr &&
		    so->so_faddr.s_un.s_addr == faddr.s_un.s_addr &&
		    so->so_fport == fport)
		   break;
	}
	...
	struct socket *so;
{
	...
	if (so->so_type == IPPROTO_ICMP) {   /* This is a "ping" reply */
	  ...
	  n = 0; /* EwokOS ioctl may not support FIONREAD */
	...
	struct mbuf *m;
{
	...
	DEBUG_MISC((dfd, " sendto()ing, addr.sin_port=%d, addr.sin_addr.s_un.s_addr=%.16s\n",
	...
	int flags;
{
	...
	so->so_laddr.s_un.s_addr = laddr; /* Ditto */
