...
/* EwokOS SDL2 has no audio queue (push) API, so the output queue is
 * emulated with a ring buffer drained by Ewok_Output_CallBack() */
#define OUT_QUEUE_SZ      17 /* buffer size in power of two */
#define OUT_QUEUE_MASK    ((1<<OUT_QUEUE_SZ)-1)
static Uint8  outQueue[1<<OUT_QUEUE_SZ];
static Uint32 outQueueRd = 0;
static Uint32 outQueueWr = 0;

static Uint32 out_queue_used(void) {
    return (outQueueWr - outQueueRd) & OUT_QUEUE_MASK;
}

void Audio_Output_Queue(Uint8* data, int len) {
    Uint32 tail;
    if (!bSoundOutputWorking || len <= 0) return;
    if (len >= (int)(1<<OUT_QUEUE_SZ)) {
        /* keep only the last part of oversized chunks */
        data += len - ((1<<OUT_QUEUE_SZ) - 1);
        len   = (1<<OUT_QUEUE_SZ) - 1;
    }
    /* drop oldest data if the queue is full */
    while (out_queue_used() + (Uint32)len >= (Uint32)(1<<OUT_QUEUE_SZ)) {
        outQueueRd = (outQueueRd + 1) & OUT_QUEUE_MASK;
    }
    tail = (1<<OUT_QUEUE_SZ) - outQueueWr;
    if ((Uint32)len <= tail) {
        memcpy(&outQueue[outQueueWr], data, len);
    } else {
        memcpy(&outQueue[outQueueWr], data, tail);
        memcpy(outQueue, data + tail, len - tail);
    }
    outQueueWr = (outQueueWr + (Uint32)len) & OUT_QUEUE_MASK;
}

Uint32 Audio_Output_Queue_Size() {
    if (bSoundOutputWorking) {
        return out_queue_used() / 4;
    } else {
        return 0;
    }
}

static void Ewok_Output_CallBack(void *userdata, Uint8 *stream, int len) {
    Uint32 used = out_queue_used();
    Uint32 n = used < (Uint32)len ? used : (Uint32)len;
    Uint32 tail = (1<<OUT_QUEUE_SZ) - outQueueRd;
    userdata = NULL;
    if (n <= tail) {
        memcpy(stream, &outQueue[outQueueRd], n);
    } else {
        memcpy(stream, &outQueue[outQueueRd], tail);
        memcpy(stream + tail, outQueue, n - tail);
    }
    outQueueRd = (outQueueRd + n) & OUT_QUEUE_MASK;
    if (n < (Uint32)len) {
        memset(stream + n, 0, len - n); /* silence */
    }
}
...
void Audio_Output_Init(void)
{
    ...
    request.callback = Ewok_Output_CallBack; /* EwokOS has no queue API */
