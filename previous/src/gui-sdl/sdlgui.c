...
int SDLGui_Init(void)
{
	...
	/* EwokOS: the SDL2 build lacks indexed (8bpp -> 32bpp) blit routines,
	 * so convert the fonts to ARGB8888 before they are ever blitted. */
	{
		SDL_Surface *tmp;
		tmp = SDL_ConvertSurfaceFormat(pSmallFontGfx, SDL_PIXELFORMAT_ARGB8888, 0);
		if (tmp) {
			SDL_FreeSurface(pSmallFontGfx);
			pSmallFontGfx = tmp;
		}
		tmp = SDL_ConvertSurfaceFormat(pBigFontGfx, SDL_PIXELFORMAT_ARGB8888, 0);
		if (tmp) {
