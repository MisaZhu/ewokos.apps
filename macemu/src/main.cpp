...
bool InitAll(const char *vmdir)
{
	...
	switch (ROMVersion) {
		...
		case ROM_VERSION_32:
			...
			FPUType = PrefsFindBool("fpu") ? 1 : 0;
			if (CPUType == 4) FPUType = 1;	// 68040 always with FPU
			// 24-bit addressing by default so System 6 boots; set the
			// "32bit" preference for System 7.5+/32-bit-clean setups
			TwentyFourBitAddressing = !PrefsFindBool("32bit");
			break;
	}
	CPUIs68060 = false;

	// 24-bit address space only holds 4MB of RAM below the ROM at 0x400000
	if (TwentyFourBitAddressing && RAMSize > 0x400000)
		RAMSize = 0x400000;
#endif
